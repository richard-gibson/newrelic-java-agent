...
class ScalaInstrumentedUnmarkedSegmentNestedAsynchronousSingleThreadTest {
  ...
  def finishedTransactionCollection(introspector :Introspector, expectedTxCount: Int = 1): Future[Unit] = {
    Thread.sleep(100)
    Future {
      while (introspector.getFinishedTransactionCount() < expectedTxCount) {
        Thread.sleep(100)
      }
    }(singleThread)
  }
